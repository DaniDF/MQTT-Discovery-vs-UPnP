...
do
    for qos in {0..2} ;
    do
        ssh -f -MS /tmp/ssh-device user@10.2.0.107 "tests/bin/device -m $mqtt_devices --mqtt-broker 10.2.0.108:1883 --qos "$qos" > tests/logs/device_test-latency_m-"$mqtt_devices"_q-"$qos"_$(date +"%Y%m%d_%H%M%S_%N").log 2>&1 < /dev/null"
