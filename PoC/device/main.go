...
func main() {
	...
	err := rpio.Open()
	if err != nil {
		log.Error("[main-device] Error while configuring GPIO: " + err.Error())
		return
	}
	defer rpio.Close()
	log.Info("[main-device] GPIO initialised")

	switchPin := rpio.Pin(gpioPin)
	switchPin.Output()
	switchPin.Low()
	log.Info("[main-device] GPIO pin " + strconv.Itoa(gpioPin) + " initialised")

	if args.DebugEnabled {
		for range gpioInitFlash {
			switchPin.High()
...
func CreateUpnpRootDevice(ctx context.Context, upnpPort int, devicePin rpio.Pin) (upnp.RootDevice, error) {
	log := ctx.Value("logger").(logging.Logger)
