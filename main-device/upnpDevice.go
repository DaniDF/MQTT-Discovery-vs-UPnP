...
func ssdp(ctx context.Context) error {
	...
	deviceXML := ""
	/*deviceXML := strings.ReplaceAll(deviceXMLTemplate, "{{IP}}", getLocalIP())
	deviceXML = strings.ReplaceAll(deviceXML, "{{PORT}}", string(upnpPort))
	deviceXML = strings.ReplaceAll(deviceXML, "{{UUID}}", upnpUUID)*/
	...
	for {
		...
		if err != nil {
			...
			if isMSearch {
				...
				response, err := handleSSDPRequest(message)
				...
				if err != nil && err.Error() == "Request not valid: ST not present" {
					...
					//<-wait //TODO in theory right but I suspect the receiver is not full compliat with the standar (MX wait time)
...
func handleSSDPRequest(message UDPPacket) (UDPPacket, error) {
	fmt.Println(message.source.IP.String() + ": " + message.message)
	...
	requestForThisDevice := false
	usn := ""
	switch st {
	case "ssdp:all", rootDevice.Device.UDN:
		requestForThisDevice = true
	...
	responseMessage := "HTTP/1.1 200 OK\r\n" +
		...
		//Add date
		"EXT:\r\n" +
		"LOCATION: " + rootDevice.Device.PresentationURL + "\r\n" +
...
func httpServer(ctx context.Context) {
	go func() {
		...
		//http.HandleFunc("/"+deviceXMLFilename, func(resp http.ResponseWriter, req *http.Request) { deviceDescriptionHandler(ctx, resp, req) })

		log.Info("Listening for request")
		if http.ListenAndServe(GetLocalIP()+":8080", nil) != nil {
			log.ErrorContext(ctx, "Error occurred while listen and serve")
		}
	}()
}

func deviceDescriptionHandler(ctx context.Context, response http.ResponseWriter, request *http.Request) {
	log := ctx.Value("logging").(logging.Logger)
